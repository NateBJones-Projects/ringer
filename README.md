...
`args_template` is an argv array, not a shell string. Ringer replaces `{taskdir}` and `{spec}` inside each argv element. `{access_args}`, `{sandbox_args}`, and `{full_access_args}` expand to multiple argv elements only when they appear as their own array item.
