...
OpenCode ships no OS sandbox, so the engine's `bin` is `engines/opencode-sandboxed.sh`: a macOS Seatbelt wrapper that leaves network and reads open but confines writes to the task dir, temp dirs, and OpenCode's own state dirs. Its `--dangerously-skip-permissions` flag only silences OpenCode's interactive prompts; Seatbelt is the actual containment. `--no-sandbox` is wired as the engine's `full_access_args`, so ringer's `allow_full_access` gate still governs escapes. Non-macOS installs need their own sandbox (or full-access mode).
