...
- 2026-07-22 — gh-pages migration impl: two GitHub Actions workflow lanes (preview-publish/cleanup, promote/dependabot; ~25 structural+security assertions each, high effort): both PASS attempt 1. Scoped tokens, fork guard, rsync protected-path excludes all correct on first try. Workflow authoring is codex's lane even though site-build is not.
...
- 2026-07-22 — gh-pages migration impl: site-build lane (rake staging dual build + config-delta gate w/ executed negative test), redirect pages (executed build check), setup doc: 3/3 PASS attempt 1. LESSON: its patch export (git add -A) swept in regenerable copy-assets outputs (en/js, fr/js) — future repo checks should exclude build-output paths from the patch or clean before export.
- 2026-07-23 — gh-pages staging asset rewrite: PASS attempt 2. LESSON IS ABOUT THE CHECK, NOT THE MODEL: my grep flagged protocol-relative `//cdn` URLs as unprefixed root-absolute, contradicting the spec and pressuring the worker to corrupt them into `/baseurl//cdn` paths. The worker's reasoning log caught it first ('the spec is unambiguous: don't prefix //'). Fixed the check mid-flight (single-slash-not-followed-by-slash + an anti-corruption assert). When a worker retries on a confusing assertion, read its reasoning before blaming the model.
...
## GPT-5.5 (codex)
- 2026-07-27 code-fix + code-feature (mpac-wizard-links, Astro 7 campaign site): 2/2 first-try at `model_reasoning_effort=medium`, 33k tok / 128s and 44k tok / 130s, worktrees mode. Both patches were byte-exact to spec with zero scope creep — including the subtle part (a CMS field that must fall back per-locale because the live Storyblok story lacks the field). Medium effort was the right dial for "spec is precise, judgment is low"; no need to pay for high here. Worktrees have no node_modules (gitignored), so the spec and the check both had to `ln -s` it from the main repo before `npm run build` — that instruction in HOW TO RUN is what made both tasks pass on attempt 1.
...
- 2026-07-21 docs (votonselisma-social-launch signup-guide): 1/1 first-try, $0 — 13KB four-platform account-creation runbook against a substance-grep check (2FA, LinkedIn prerequisite, handle fallbacks, verification steps). Structured checklist writing is squarely in its lane; second consecutive first-try pass on probation, one more docs/mechanical-copy pass earns proven.

## poolside/laguna-s-2.1:free
- 2026-07-28 — code-fix (3-line CSS tweak in one Astro component, votonselisma): FAIL both attempts, 45k tokens, zero edits produced. Log shows `reason: "length"` with reasoning=32000 / output=0 — it read the target file, then burned the entire output budget on reasoning and never wrote anything. Do not route it at nuanced style/judgment work; if auditioned again, give it something with a single unambiguous mechanical answer.

## GLM 5.2 (openrouter/z-ai/glm-5.2)
- 2026-07-28 — code-feature x3 + code-fix x1 (votonselisma refinements): 4/4 PASS, 28k-69k tokens, 1-2 attempts. Strong on self-contained module work: the French-typography helper passed first try with a correct URL/time-safe implementation and its own test suite, and a Storyblok loader mirroring an existing reference file passed on retry. Two cautions: (a) on a content-correction task it silently rewrote an established brand slogan while fixing an adjacent factual error — scope content specs tightly and always diff brand copy; (b) it can rat-hole on environment problems. On the same 3-line CSS task the free model failed, GLM spent its whole run debugging a node_modules symlink and sandbox xattrs instead of editing CSS, because the check required `astro check`.
- Orchestrator lesson from the same run (my fault, not the model's): do not require a toolchain command in the check for a pure-CSS change in an isolated worktree. Fresh worktrees have no node_modules, and the symlink workaround turns a trivial edit into a sandbox-debugging task. Use text assertions for style-only work and verify the aesthetics visually yourself.
- Worktree gotcha confirmed: `.gitignore` entries ending in `/` (e.g. `node_modules/`) match directories, so a node_modules SYMLINK is NOT ignored and `git add -A` sweeps it into the exported patch. Remove any such symlink before the validator stages, or the ownership check fails on it.
