...
- 2026-08-26 (code-feature, phone-cop-voicemail-fetch + vm-pull-gate runs, GLM 5.2): repo-feature tasks cannot direct-edit a repo from opencode workers. engines/opencode-sandboxed.sh (Seatbelt) denies every write outside the taskdir BY DESIGN — one worker verified EPERM across bash, python, and the write tool; the machine's ~/.config/opencode ask-allowlist is a second belt behind it — and the kit's `writable_roots` engine_args is codex syntax that opencode ignores. Both workers built CORRECT code, verified it in mirrors (10/10 vitest; 262-test unittest suite green), staged the files, and wrote CRPs saying exactly this — then burned 2/2 attempts each because the check validated an untouched repo, and its build step masked the required-paths story ("No test files found"). Orchestrator/kit bug — do not read these four FAILs against GLM. Fixed same day: check_repo_feature.py now installs files staged at output/repo/<repo-relative-path> into --repo before validating (--stage-dir; owned paths only; all-or-nothing), pre-build failures print the full story plus a staging hint, and the kit spec teaches the fallback (mirror inside the taskdir, never $TMPDIR — the wrapper's EXIT trap wipes it). RULING (sandbox-safety vs utility, per the workers' CRPs): keep the sandbox tight — do NOT widen the Seatbelt profile or opencode.jsonc write roots; check-side harvest is the general fix for any write-confined engine. Probe-verified same day (run opencode-repo-write-fix, one-task manifest): GLM 5.2 hit the write block, followed the spec's fallback unprompted (mirror → 4/4 tests → staged both owned files), the check harvested them and ran the real suite in the real repo — PASS attempt 1, 14.8k tokens, 17s.
