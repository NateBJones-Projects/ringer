...
- 2026-09-02 (colourfully-site-fixes, 3 code-fix tasks on a 114MB Astro 5 site, worktrees mode): all three opencode/GLM-5.2 workers produced ZERO output and hit the 1800s timeout on BOTH attempts (3650s wall each). Not a model failure and not the retry-warm-up pattern noted on 2026-07-09 — the harness never emitted a byte. Isolated with three probes on the same trivial task (create public/robots.txt): git worktree of the repo → 0 bytes in 240s; a NORMAL git clone of the same repo → 0 bytes in 150s; a plain directory holding only src/ with no .git → completed and wrote the file. So it is not concurrency (a single run stalls too) and not worktrees specifically (a plain clone stalls too); it tracks with the presence of .git on a repo this size. Codex ran the IDENTICAL manifest 4/4 PASS at 77-129s per task. Lesson: a lane proof in a tiny scratch dir does NOT qualify opencode for repo work — audition it on a representative checkout before assigning a batch, and give opencode lanes a short timeout_s so a stall costs minutes, not an hour.
...
## Process lessons (2026-09-02, agent skills reach workers)
- **A globally installed agent skill reaches a Ringer worker, and binds without being asked.** Measured, run `ste-worker-probe-20260902T214711Z-p13412`: an `asd-ste100` (Simplified Technical English) skill was installed to the cross-agent path `~/.agents/skills/`, then a one-task probe asked a Codex worker to list its available skills and to write 120-180 words of ordinary prose. The spec never mentioned STE or style. The worker listed `asd-ste100` among its own skills and wrote prose scoring **1.20 violations per 100 words** on attempt 1. Calibration on the same machine: unstyled technical prose (this repo's README and CONTRIBUTING) scores 3.5-5.6, and the skill's own STE-written README scores 1.24. The worker also emitted zero em dashes where this repo's README carries 72. So worker behavior is shapeable through the worker CLI's global skill directory, with no change to `ringer.py` and no change to the spec.
- **A skill that ships a linter turns a style rule into a real check.** The same skill ships a stdlib-only linter with `--fail-over <n>` / `--fail-shape <n>`, which exit 1 above a threshold. That makes a writing standard an executed check rather than a hope, which is the difference the tool exists to make. Footgun: WITHOUT those flags the linter always exits 0, so `ste-lint.py report.md` as a bare check is a check that cannot fail. Set the threshold from calibration on your own prose, not from a guess.
- **Project-scoped skills should NOT reach workers — reasoned, not measured.** A BMAD-METHOD install (`npx bmad-method install`) wrote 49 skills to `<repo>/.claude/skills` and 49 to `<repo>/.agents/skills`, i.e. inside the project. A Ringer worker runs with cwd set to its task directory, which is outside that repo (a scratch dir, or a worktree at another path), so project-scoped skills should be invisible to it while global ones are not. This was NOT probed — it is inferred from the install paths and from where workers run. If it matters to you, probe it before relying on it. The practical consequence either way: to give a worker a persona, put the persona text in the manifest spec, because the spec is the one channel a stateless worker is guaranteed to read.
