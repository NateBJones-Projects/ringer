...
Worked example, because this exact conflation has happened: **"Grok Build" is a harness, never a model and never a lab.** Model availability is versioned and dynamic; Ringer records a selectable model ID and, when present, the CLI's reported usage key. A scoreboard row named only after the harness is a taxonomy bug.
...
Every primary scoreboard table uses exactly these columns, in this order: **Model | Lab | Harness | API/Plan | Invocation | Performance | Tasks | First try | Pass | Tokens (median) | Speed (median) | Last used | Notes**. Model contains the registry display name plus the reasoning-effort suffix required below, never a raw slug or slug parenthetical. Invocation says whether identity was a harness-reported match, harness-reported without an older requested field, a pinned request without a report, a legacy label, unattributed, mixed, or mismatched. Its detail retains the exact requested and reported strings. Performance is the statistical `proven`/`probation` tier; it does not prove model routing. Unregistered rows derive a readable display name and keep the raw slug only in the diagnostics pointer. Notes is the most recent dated bullet read from `docs/MODEL-NOTES.md` at render time; the other dated bullets may appear in expandable detail. Attempts and failed counts are detail data, not primary columns.

Registry entries may declare `report_aliases` when a harness accepts one selectable model ID but reports a different backend or billing key. Alias equivalence prevents a false mismatch while preserving both raw strings. It never makes an unreported invocation harness-attested.
...
2. **Grok Build CLI:** Run `grok --help` and inspect the local model cache or authenticated `grok models` output, then cross-check <https://docs.x.ai/build/overview>. Headless JSON includes per-model `modelUsage` keys. Record the selectable `-m` ID as the requested model, preserve the raw usage key as the report, and declare a reviewed `report_aliases` mapping only when the local catalog and run evidence establish that they are the same route.
