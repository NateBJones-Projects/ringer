...
1. **Codex CLI:** Open any worker log from the run and read the self-reported `model:` and `provider:` lines in the Codex header. Record the model slug exactly, then cross-check it against <https://learn.chatgpt.com/docs/models>. The self-report wins when it differs from the manifest or config; the attempt row retains the resolved slug as `expected_model` so the drift is visible.
